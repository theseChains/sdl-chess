        ...
        int pawnRow, int pawnColumn)
{
    ...
    if ((pawn.getColor() == PieceColor::white && pawnRow != 0) ||
        (pawn.getColor() == PieceColor::black && pawnRow != 7))
        return false;
...
void drawPromotionPieces(const std::array<std::array<Tile, 8>, 8>& board,
        TextureTable& textureTable, Renderer& renderer, const Move& lastMove)
{
    ...
    Piece queen{ PieceType::queen, color,
        textureTable[{ color, PieceType::queen }], { tileRow, tileColumn } };
    Piece rook{ PieceType::rook, color,
        textureTable[{ color, PieceType::rook }],
            { tileRow + (color == PieceColor::white ? 100 : -100), tileColumn } };
    Piece bishop{ PieceType::bishop, color,
        textureTable[{ color, PieceType::bishop }],
            { tileRow + (color == PieceColor::white ? 200 : -200), tileColumn } };
    Piece knight{ PieceType::knight, color,
        textureTable[{ color, PieceType::knight }],
            { tileRow + (color == PieceColor::white ? 300 : -300), tileColumn } };

    renderer.setDrawColor({ 255, 255, 255, 255 });
    renderer.fillAndDrawRect({ queen.getPosition().second,
        queen.getPosition().first,
            constants::windowWidth / constants::boardSize,
                constants::windowHeight / constants::boardSize });
    renderer.fillAndDrawRect({ rook.getPosition().second,
        rook.getPosition().first,
            constants::windowWidth / constants::boardSize,
                constants::windowHeight / constants::boardSize });
    renderer.fillAndDrawRect({ bishop.getPosition().second,
        bishop.getPosition().first,
            constants::windowWidth / constants::boardSize,
                constants::windowHeight / constants::boardSize });
    renderer.fillAndDrawRect({ knight.getPosition().second,
        knight.getPosition().first,
            constants::windowWidth / constants::boardSize,
                constants::windowHeight / constants::boardSize });
